...
test.describe('Clientes Principal Page Tests', () => {
  ...
  test.afterAll(async ({  }) => {
    ...
    test('IN-94: Admin > Clients > New Client > Billing Address > Validar que permita el campo State/Province con 1 carácter', {
      tag: ['@clients', '@regression']
    }, async () => {
        let clientData = DataGenerator.generateClientData();
        const singleCharName = 'a';
        ...
        await test.step('Crear cliente con 1 carácter en el campo State/Province', async () => {
          await userTab.CreateClients().fillNameField(clientData.company.name);
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingStateField(singleCharName);
          await userTab.CreateClients().clickSaveButton();
        });
        await test.step('Verificar que el cliente se creó exitosamente', async () => {
          expect(await userTab.CreateClients().isCreateConfirmationTextVisible()).toBeTruthy();
    ...
    test('IN-95: Admin > Clients > New Client > Billing Address > Validar que permita el campo State/Province con 255 caracteres', {
      tag: ['@regression', '@clients']
    }, async () => {
        let clientData = DataGenerator.generateClientData();
        ...
        await test.step('Crear cliente con 255 caracteres en el campo State/Province', async () => {
          await userTab.CreateClients().fillNameField(clientData.company.name);
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingStateField(maxCharName);
          await userTab.CreateClients().clickSaveButton();
        });
        await test.step('Validar que el cliente aparece en la lista', async () => {
          await userTab.BaseNavigationPage().clickClients();
    ...
    test.fixme('IN-96: Admin > Clients > New Client > Billing Address > Validar que NO permita el campo State/Province con 256 caracteres', {
      tag: ['@regression', '@clients', '@negative']
        ...
        try {
          ...
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingStateField(exceedCharName);
          await userTab.CreateClients().clickSaveButton();
          const wasCreated = await userTab.CreateClients().isCreateConfirmationTextVisible();
          if (wasCreated) {
            clientWasCreated = true;
    ...
    test.fixme('IN-291: Admin > Clients > New Client > Billing Address > Validar que NO permita el campo City con SOLO caracteres numericos', {
      tag: ['@regression', '@clients', '@negative']
         ...
         const clientData = DataGenerator.generateClientData();
        const exceedCharName = '1234567890';
        let clientWasCreated = false;
        try {
          await userTab.BaseNavigationPage().clickClients();
          await userTab.Clients().clickNewClientButton();
    ...
    test('IN-97: Admin > Clients > New Client > Billing Address > Validar que permita el campo Postal Code con 1 carácter permitido', {
      tag: ['@clients', '@regression']
    }, async () => {
        let clientData = DataGenerator.generateClientData();
        const singleCharName = '1';
        ...
        await test.step('Crear cliente con 1 carácter en el campo Postal Code', async () => {
          await userTab.CreateClients().fillNameField(clientData.company.name);
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingPostalCodeField(singleCharName);
          await userTab.CreateClients().clickSaveButton();
        });
        await test.step('Verificar que el cliente se creó exitosamente', async () => {
          expect(await userTab.CreateClients().isCreateConfirmationTextVisible()).toBeTruthy();
    ...
    test('IN-98: Admin > Clients > New Client > Billing Address > Validar que permita el campo Postal Code con 10 caracteres permitidos', {
      tag: ['@regression', '@clients']
    }, async () => {
        let clientData = DataGenerator.generateClientData();
        const maxCharName = '1'.repeat(10);
        await test.step('Ir al modulo Clients y hacer clic en "Nuevo Cliente"', async () => {
          await userTab.BaseNavigationPage().clickClients();
          await userTab.Clients().clickNewClientButton();
        });
        await test.step('Crear cliente con 10 caracteres en el campo Postal Code', async () => {
          await userTab.CreateClients().fillNameField(clientData.company.name);
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingPostalCodeField(maxCharName);
          await userTab.CreateClients().clickSaveButton();
        });
        await test.step('Validar que el cliente aparece en la lista', async () => {
          await userTab.BaseNavigationPage().clickClients();
    ...
    test.fixme('IN-99: Admin > Clients > New Client > Billing Address > Validar que NO permita el campo Postal Code con 11 caracteres', {
      tag: ['@regression', '@clients', '@negative']
        ...
        const clientData = DataGenerator.generateClientData();
        const exceedCharName = '5'.repeat(11);
        let clientWasCreated = false;
        try {
          await userTab.BaseNavigationPage().clickClients();
          await userTab.Clients().clickNewClientButton();
          ...
          await userTab.CreateClients().fillFirstNameField(clientData.contact.firstName);
          await userTab.CreateClients().fillLastNameField(clientData.contact.lastName);
          await userTab.CreateClients().fillBillingPostalCodeField(exceedCharName);
          await userTab.CreateClients().clickSaveButton();
          const wasCreated = await userTab.CreateClients().isCreateConfirmationTextVisible();
          if (wasCreated) {
            clientWasCreated = true;
            console.log('FALLO: El sistema permitió crear cliente con 11 caracteres');
          } else {
            console.log('CORRECTO: El sistema NO permitió crear cliente con 11 caracteres');
          }
          expect(wasCreated).toBeFalsy();
        } finally {
          if (clientWasCreated) {
