...
test.describe('Products Principal Page Tests', () => {
  ...
  test.afterAll(async ({  }) => {
    ...
    test('IN-230: Admin > Products > New Product > Validar que permita el campo Max Quantity con 1 dígito', {
      tag: ['@products', '@regression']
    }, async () => {
        let productData = DataGenerator.generateProductData();
        const singleChar = '1';
        ...
        await test.step('Crear producto con 1 digito en el campo Max Quantity', async () => {
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillGenericNumberField('Max Quantity', singleChar);
          await userTab.CreateProducts().clickSaveButton();
        });
        await test.step('Verificar que el producto se creó exitosamente', async () => {
          expect(await userTab.CreateProducts().isCreateConfirmationTextVisible()).toBeTruthy();
    ...
    test('IN-231: Admin > Products > New Product > Validar que permita el campo Max Quantity con 20 dígitos', {
      tag: ['@regression', '@products']
    }, async () => {
        let productData = DataGenerator.generateProductData();
        const maxCharName = '1'.repeat(20);
        ...
        await test.step('Crear producto con 20 caracteres en el campo Max Quantity', async () => {
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillGenericNumberField('Max Quantity', maxCharName);
          await userTab.CreateProducts().clickSaveButton();
        });
        await test.step('Validar que el producto aparece en la lista', async () => {
          await userTab.BaseNavigationPage().clickProducts();
    ...
    test('IN-232: Admin > Products > New Product > Validar que NO permita el campo Max Quantity con 21 dígitos', {
      tag: ['@regression', '@products', '@negative']
        ...
        try {
          ...
          await userTab.Products().clickNewProductButton();
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillGenericNumberField('Max Quantity', exceedCharName)
          await userTab.CreateProducts().clickSaveButton();
          const wasCreated = await userTab.CreateProducts().isCreateConfirmationTextVisible();
          if (wasCreated) {
            productWasCreated = true;
    ...
    test('IN-233: Admin > Products > New Product > Validar que NO permita el campo Max Quantity con caracteres especiales', {
      tag: ['@regression', '@products', '@negative']
        ...
        try {
          ...
          await userTab.Products().clickNewProductButton();
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillGenericNumberField('Max Quantity', specialCharName)
          await userTab.CreateProducts().clickSaveButton();
          const wasCreated = await userTab.CreateProducts().isCreateConfirmationTextVisible();
          if (wasCreated) {
            productWasCreated = true;
    ...
    test('IN-298: Admin > Products > New Product > Validar que permita un campo Image URL con 1 carácter', {
      tag: ['@products', '@regression']
    }, async () => {
        let productData = DataGenerator.generateProductData();
        const singleCharName = 'a';
        ...
        await test.step('Crear producto con 1 carácter en el campo Image URL', async () => {
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillImageUrlField(singleCharName);
          await userTab.CreateProducts().clickSaveButton();
        });
        await test.step('Verificar que el producto se creó exitosamente', async () => {
          expect(await userTab.CreateProducts().isCreateConfirmationTextVisible()).toBeTruthy();
    ...
    test('IN-299: Admin > Products > New Product > Validar que permita un campo Image URL con 255 caracteres', {
      tag: ['@regression', '@products']
    }, async () => {
        let productData = DataGenerator.generateProductData();
        const maxCharName = 'a'.repeat(255);
        ...
        await test.step('Crear producto con 255 caracteres en el campo Image URL', async () => {
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillImageUrlField(maxCharName);
          await userTab.CreateProducts().clickSaveButton();
        });
        await test.step('Validar que el producto aparece en la lista', async () => {
          await userTab.BaseNavigationPage().clickProducts();
    ...
    test('IN-300: Admin > Products > New Product > Validar que NO permita un campo Image URL con 256 caracteres', {
      tag: ['@regression', '@products', '@negative']
        ...
        try {
          ...
          await userTab.Products().clickNewProductButton();
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillImageUrlField(exceedCharName);
          await userTab.CreateProducts().clickSaveButton();
          const wasCreated = await userTab.CreateProducts().isCreateConfirmationTextVisible();
          if (wasCreated) {
            productWasCreated = true;
    ...
    test('IN-301: Admin > Clients > New Client > Validar que NO permita un campo Image URL con SOLO caracteres especiales', {
      tag: ['@regression', '@products', '@negative']
        ...
        try {
          ...
          await userTab.Products().clickNewProductButton();
          await userTab.CreateProducts().fillItemField(productData.name);
          await userTab.CreateProducts().fillImageUrlField(specialCharName);
          await userTab.CreateProducts().clickSaveButton();
          const wasCreated = await userTab.CreateProducts().isCreateConfirmationTextVisible();
          if (wasCreated) {
            productWasCreated = true;
